import { NavigateEvent, navigation } from "@/@types/navigationApi";
...
function isBackNavigation(navigateEvent: NavigateEvent) {
  if (navigateEvent.navigationType === 'push' || navigateEvent.navigationType === 'replace') {
    return false;
  }
  if (
    navigateEvent.destination.index !== -1 &&
    navigateEvent.destination.index < (window as any).navigation.currentEntry.index
  ) {
    return true;
  }
  return false;
...
export async function onLinkNavigate(callback: any) {
  (window as any).addEventListener('navigate', (event: NavigateEvent) => {
...
export function getLink(href: string) {
  ...
  return [...(window as any).document.querySelectorAll('a')].find((link) =>
    link.href === fullLink
  );
}
...
export function transitionHelper({
  ...
  (transition as any).finished.finally(() =>
